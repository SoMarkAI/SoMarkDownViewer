...
class SoMarkDownViewerApp {
    constructor() {
        this.elements = {
            ...
            autosaveEnabledCheckbox: document.getElementById("autosave-enabled-checkbox"),
            autosaveIntervalInput: document.getElementById("autosave-interval-input"),
            languageSelect: document.getElementById("language-select"),
            historyPanel: document.getElementById("history-panel"),
            historyBackdrop: document.getElementById("history-backdrop"),
            closeHistoryBtn: document.getElementById("history-close-btn"),
            historyList: document.getElementById("history-list")
        };

        this.i18n = new I18nManager();
        this.store = new LocalDocStore();
        this.session = new EditorSession({ store: this.store });
        ...
        this.autosave = new AutosaveController({
            store: this.store,
            onSave: () => this.saveCurrentDocument()
        });
        this.historyPanel = new HistoryPanel({
            elements: this.elements,
            store: this.store,
            i18n: this.i18n,
            onOpenDocument: (record) => this.loadHistoryDocument(record),
            onDeleteActiveDocument: () => this.loadExampleDocument()
        });
    ...
    initI18n() {
        this.i18n.onChange(() => {
            ...
            this.updateSaveStatus(this.elements.saveStatus.dataset.status || "saved");
    ...
    initSettingsUI() {
        ...
        this.syncAutosaveSettingsUI();
    }

    syncSettingsUI() {
        this.elements.filePath.textContent = this.session.getDisplayName();
        this.elements.tocLevelValue.textContent = String(this.settings.tocLevel);
        this.elements.languageSelect.value = this.settings.language;
        this.historyPanel.setActiveDocumentId(this.session.getStorageId());
        this.syncAutosaveSettingsUI();
    }
    ...
    syncAutosaveSettingsUI() {
        const autosaveSettings = this.autosave.getSettings();
        this.elements.autosaveEnabledCheckbox.checked = autosaveSettings.enabled;
        this.elements.autosaveIntervalInput.value = String(autosaveSettings.intervalSeconds);
    }
    ...
    async handleLocalFileSelect() {
        ...
        try {
            ...
            this.session.setLocalFile(selectedFile.name);
    ...
    initDocumentActionEvents() {
        ...
        document.addEventListener("keydown", (event) => {
            ...
            event.preventDefault();
            this.autosave.cancel();
            this.saveCurrentDocumentWithFeedback();
        });
    }

    createNewFile() {
        const result = this.session.createBlankDocument();
        if (!result.ok) {
    ...
    saveCurrentDocument() {
        const result = this.session.save(this.elements.source.value);
        if (!result.ok) {
            this.updateSaveStatus("failed");
            return result;
        }
    ...
    saveCurrentDocumentWithFeedback() {
        this.updateSaveStatus("pending");
        window.setTimeout(() => {
    ...
    downloadCurrentFile() {
        downloadTextFile({
            content: this.elements.source.value,
            fileName: this.session.getDownloadBaseName()
        });
    }
    ...
    initSettingPanelEvents() {
        ...
        this.elements.autosaveEnabledCheckbox.addEventListener("change", () => {
            this.autosave.updateSettings({
                enabled: this.elements.autosaveEnabledCheckbox.checked
            });
        });

        this.elements.autosaveIntervalInput.addEventListener("input", () => {
            const intervalSeconds = Number.parseFloat(this.elements.autosaveIntervalInput.value);
            this.autosave.updateSettings({
                intervalSeconds: Number.isFinite(intervalSeconds) ? intervalSeconds : 3
            });
        });
    ...
    initHistoryPanelEvents() {
        this.elements.historyOpenBtn.addEventListener("click", () => this.openHistoryPanel());
        this.elements.closeHistoryBtn.addEventListener("click", () => this.closeHistoryPanel());
        this.elements.historyBackdrop.addEventListener("click", () => this.closeHistoryPanel());
        document.addEventListener("keydown", (event) => {
            if (event.key === "Escape") {
    ...
    async loadInitialContent() {
        const currentPath = this.resolveMarkdownPath();
        ...
        try {
            ...
            if (currentPath === "docs/example.md") {
                this.session.setExample(text, currentPath);
            } else {
                this.session.setLocalFile(currentPath);
            }
        } catch (error) {
            const errorContent = `# ${this.i18n.t("errors.loadFile", { path: currentPath })}\n`;
            this.elements.source.value = errorContent;
            if (currentPath === "docs/example.md") {
                this.session.setExample(errorContent, currentPath);
            } else {
                this.session.setLocalFile(currentPath);
            }
        }
    ...
    async loadExampleDocument() {
        const examplePath = "docs/example.md";
        const targetUrl = new URL(examplePath, window.location.origin);
        let text = "";
        try {
            const response = await fetch(targetUrl.toString());
            if (!response.ok) {
            ...
            text = `# ${this.i18n.t("errors.loadFile", { path: examplePath })}\n`;
        }

        this.session.setExample(text, examplePath);
        this.elements.source.value = text;
        this.syncSettingsUI();
        this.updateLineNumbers();
        this.renderMarkdown();
    ...
    loadHistoryDocument(record) {
        this.session.setHistoryDocument(record);
        this.elements.source.value = record.content;
        this.syncSettingsUI();
        this.updateLineNumbers();
        this.renderMarkdown();
        this.updateSaveStatus("saved");
    ...
    updateSaveStatus(status) {
        const statusIcons = {
            pending: '<svg class="save-status__icon save-status__icon--pending" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 1 1-2.64-6.36"></path></svg>',
            saved: '<svg class="save-status__icon save-status__icon--saved" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"></path></svg>',
            failed: '<svg class="save-status__icon save-status__icon--failed" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"></circle><path d="M12 8v5"></path><path d="M12 16h.01"></path></svg>'
        };
        const normalizedStatus = statusIcons[status] ? status : "saved";
        this.elements.saveStatus.dataset.status = normalizedStatus;
        this.elements.saveStatus.className = `save-status save-status--${normalizedStatus}`;
        this.elements.saveStatus.innerHTML = statusIcons[normalizedStatus];
        this.elements.saveStatus.setAttribute("aria-label", this.i18n.t(`saveStatus.${normalizedStatus}`));
        this.elements.saveStatus.setAttribute("title", this.i18n.t(`saveStatus.${normalizedStatus}`));
    }
